...
def word_score(word):
    ...
    if word_length >= 10:
        for letter in word:
            score += 3
    elif word_length >=7 and word_length <= 9:
        for letter in word:
            score += 2
    elif word_length >=3 and word_length <= 6:
        for letter in word:
            score += 1
    return score
...
def update_score(player_info, word):
    ...
    """
    newScore = word_score(word)
    player_info[1] += newScore
...
def num_words_on_board(board, words):
    ...
    count = 0
    for word in words:
        result = board_contains_word(board, word)
        if result:
            count += 1
    return count
